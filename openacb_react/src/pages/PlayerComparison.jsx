...
function PlayerSummary({ record, bio, photoUrl, colorClass }) {
  ...
  return (
    <article className={playerToolStyles.playerSummary}>
      <div className="flex flex-col sm:flex-row gap-4 items-center sm:items-start">
        <div className={`${playerToolStyles.comparisonPortrait} border-2 ${colorClass}`}>
          <span className={playerToolStyles.portraitFallback}>Sin foto</span>
          {photoUrl && (
            <img
              src={photoUrl}
              alt=""
              className={playerToolStyles.comparisonImage}
          ...
          <div className={playerToolStyles.summaryMetrics}>
            {[
              ['PPP', record.ppg],
              ['TS%', record.ts, 'ts'],
              ['USG%', record.usg, 'usg'],
              ...
              <div key={label} className={playerToolStyles.summaryMetric}>
  ...
  return (
    <div className={`${playerToolStyles.evidenceCard} ${playerToolStyles.evidenceCardPadded}`}>
      <div className="flex items-start justify-between gap-3 mb-2">
        ...
        {canUsePos && (
          <div className="segmented-control text-xs shrink-0" role="group" aria-label="Referencia del radar">
            <button
              type="button"
              onClick={() => onReferenceChange('league')}
              aria-pressed={!usePos}
              className={`segmented-option ${!usePos ? 'segmented-option-active' : ''}`}
            >
              Liga
            </button>
            <button
      ...
      <svg viewBox={`0 0 ${size} ${size}`} className="w-full max-w-[360px] mx-auto" overflow="visible">
        ...
        <path d={pathFor(valuesA)} fill="rgba(240,132,94,0.18)" stroke="#f0845e" strokeWidth={1.8} />
        <path d={pathFor(valuesB)} fill="rgba(59,130,246,0.12)" stroke="#3b82f6" strokeWidth={1.8} />
        {valuesA.map((v, i) => {
...
function MetricComparison({ playerA, playerB }) {
  return (
    <div className={playerToolStyles.evidenceCard}>
      ...
      <div className={playerToolStyles.tableFrame} tabIndex={0} aria-label="Tabla de comparación estadística">
        <table className="data-table">
          ...
          {metricGroups.map(group => (
            <tbody key={group.title}>
              <tr className="bg-acb-50/70 border-b border-acb-100">
              ...
              {group.metrics.map(metric => {
                ...
                return (
                  <tr key={metric.key} className="data-table-row border-b border-acb-100">
...
function OnOffComparison({ playerA, playerB, loadLineupsForSeason, lineupsCache, loadingLineups }) {
  ...
  return (
    <div className={playerToolStyles.evidenceCard}>
      ...
      {!hasAny && !anyLoading && (
        <div className={playerToolStyles.evidenceMeta}>No se encontraron datos de alineaciones para estos jugadores.</div>
      )}
      {hasAny && (
        <div className={playerToolStyles.tableFrame} tabIndex={0} aria-label="Tabla de impacto en pista">
          <table className="data-table">
            <thead>
              <tr className="text-xs font-semibold text-acb-600 uppercase border-b border-acb-300 bg-acb-100">
                <th className="data-table-head data-table-identity data-table-sticky data-table-sticky-head data-col-player bg-acb-100">Jugador</th>
            ...
            <tbody className="divide-y divide-acb-100">
              {rows.filter(r => r.row?.found).map(({ id, record, row, color }) => {
                ...
                return (
                  <tr key={id} className="data-table-row border-b border-acb-100">
...
export default function PlayerComparison({
  ...
  return (
    <PlayerToolShell
        ...
        actions={playerA && playerB ? (
          <div className="flex gap-2 flex-wrap">
            <Link
              ...
              className={`${playerToolStyles.secondaryButton} text-xs`}
            >
      ...
      <div className={playerToolStyles.comparisonPickerGrid}>
        <div className={playerToolStyles.comparisonPicker}>
          <PlayerCombobox
            id="compare-player-a"
            label="Jugador A"
        ...
        <button
          ...
          className={playerToolStyles.swapButton}
          title="Intercambiar jugadores"
        >
          <ArrowLeftRight size={17} strokeWidth={1.8} aria-hidden="true" />
        </button>
        <div className={playerToolStyles.comparisonPicker}>
          <PlayerCombobox
            id="compare-player-b"
      ...
      {playerA && playerB ? (
        <>
          <div className={playerToolStyles.comparisonSubjects}>
            <PlayerSummary
              record={playerA}
              bio={bioA}
              photoUrl={photoA}
