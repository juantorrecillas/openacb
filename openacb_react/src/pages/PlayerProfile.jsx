...
import { Loader2, GitCompareArrows, Flame, Download, ArrowRight } from 'lucide-react'
...
function archetypeTextClass(colorClasses) {
  return typeof colorClasses === 'string'
    ? colorClasses.split(/\s+/).find(className => className.startsWith('text-')) || ''
    : ''
}
...
function PlayerHeader({ records, photoUrl, bio, selectedSeason, selectedRecord }) {
  ...
  const totalGames = records.reduce((sum, record) => sum + (record.games || 0), 0)
  const seasonSpan = seasons.length > 1
    ? `${seasonLabel(seasons[0])}—${seasonLabel(seasons.at(-1))}`
    : seasonLabel(seasons[0])

  return (
    <section className={playerToolStyles.identityDossier} aria-label={`Ficha de ${getPlayerDisplayName(latest)}`}>
      <div className={playerToolStyles.identityLead}>
        <div className={playerToolStyles.portraitFrame}>
          <span className={playerToolStyles.portraitFallback}>Sin foto</span>
          {photoUrl && (
            <img
              src={photoUrl}
              alt=""
              className={playerToolStyles.portrait}
              onLoad={event => { event.currentTarget.dataset.loaded = 'true' }}
              onError={event => { event.currentTarget.hidden = true }}
            />
          )}
        </div>
        <div className={playerToolStyles.identityCopy}>
          <h2 className={playerToolStyles.identityName}>{getPlayerDisplayName(latest)}</h2>
          {bio?.position && <span className={playerToolStyles.identityRole}>{bio.position}</span>}
          <p className={playerToolStyles.identityContext}>
            {activeRecord.team} · {seasonLabel(activeRecord.season)}
          </p>
        </div>
      </div>

      <dl className={playerToolStyles.identityFacts}>
        <div className={playerToolStyles.identityFact}>
          <dt>Altura</dt>
          <dd data-numeric="true">{bio?.heightM ? `${parseFloat(bio.heightM).toFixed(2).replace('.', ',')} m` : '—'}</dd>
        </div>
        <div className={playerToolStyles.identityFact}>
          <dt>Edad en la temporada</dt>
          <dd data-numeric="true">{age != null ? `${age} años` : '—'}</dd>
        </div>
        <div className={playerToolStyles.identityFact}>
          <dt>Trayectoria ACB</dt>
          <dd title={teams.join(', ')}>{teams.length} equipo{teams.length !== 1 ? 's' : ''} · {seasonSpan}</dd>
        </div>
        <div className={playerToolStyles.identityFact}>
          <dt>Partidos registrados</dt>
          <dd data-numeric="true">{totalGames}</dd>
        </div>
      </dl>
    </section>
  )
...
function CareerTable({ records, bio, tab = 'basic', onTabChange }) {
  ...
  return (
    <section className={playerToolStyles.dataSection}>
      <div className={playerToolStyles.sectionHeader}>
        <div>
          <h2>Trayectoria</h2>
        </div>
          <div className="flex items-center gap-2 ml-auto">
          <div className="segmented-control" role="group" aria-label="Vista de trayectoria">
            <button
          ...
          <button
            ...
            className={playerToolStyles.secondaryButton}
            title="Descargar CSV"
          >
      ...
      <div className={playerToolStyles.tableFrame} tabIndex={0} aria-label="Trayectoria del jugador">
                ...
                {cols.map(c => {
                  ...
                  return (
                    <td key={c.key} className={`px-3 py-2 whitespace-nowrap ${
                      ...
                      {c.key === 'careerRole' ? (
                        <span
                          className={`${playerToolStyles.roleWord} ${archetypeTextClass(r.careerRole.color)}`}
...
function PercentileProfile({ player, reference = 'league', onReferenceChange }) {
  ...
  return (
    <div className={`${playerToolStyles.evidenceCard} ${playerToolStyles.evidenceCardPadded}`}>
...
function RadarChart({ player, usePos }) {
  ...
  if (values.some(value => value == null)) {
    return (
      <div className="flex min-h-64 items-center justify-center border-block border-acb-200 bg-acb-50 px-6 text-center text-sm text-acb-500" role="status">
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full max-w-[320px] mx-auto" overflow="visible">
      ...
      {/* Player fill */}
      <path d={playerPath} fill="rgba(254,89,23,0.16)" stroke="#fe5917" strokeWidth={2} />
      {/* Player dots */}
      {values.map((v, i) => {
        const [px, py] = point(i, Math.min(v, 100))
        return <circle key={i} cx={px} cy={py} r={3.5} fill="#fe5917" stroke="white" strokeWidth={1.5} />
      })}
      {/* Labels */}
      ...
      <div className="flex flex-col md:flex-row md:items-start gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="segmented-control" role="group" aria-label="Referencia del radar">
              <button
                onClick={() => onReferenceChange?.('league')}
                aria-pressed={!usePos}
                className="segmented-option"
        ...
        {/* Archetype */}
        <div className={playerToolStyles.archetypePane}>
          <span className="text-xs font-semibold text-acb-500 uppercase tracking-wider mb-2">Arquetipo</span>
          <div className={`text-xl font-bold ${archetypeTextClass(archetype.color)}`}>{archetype.name}</div>
          <p className="mt-2 text-xs leading-5 text-acb-600">{archetype.desc}</p>
...
function ShootingStatsCard({ player, shotTab = 'own', onShotTabChange }) {
  ...
  return (
    <div className={playerToolStyles.evidenceCard}>
      <div className={playerToolStyles.evidenceHeader}>
        <h3 className="font-semibold text-acb-900">Tiro por Zona</h3>
        <div className="flex items-center gap-2">
          {hasRivalData && (
            <div className="segmented-control" role="group" aria-label="Vista de tiro">
              <button
                onClick={() => onShotTabChange?.('own')}
                aria-pressed={shotTab === 'own'}
                className="segmented-option"
              >
                Tiro Propio
              </button>
      ...
      <p className={`${playerToolStyles.evidenceMeta} text-xs text-acb-500`}>
        {shotTab === 'rival' && (
          <> Dif. = tiro rival con el jugador en pista vs. media del equipo (negativo = mejor defensa)</>
        )}
      </p>
      <div className={playerToolStyles.tableFrame} tabIndex={0} aria-label="Tabla de tiro por zona">
        {shotTab === 'own' ? (
          <table className="data-table">
            <thead>
            ...
            <tbody className="divide-y divide-acb-100">
              {zones.map(z => {
                ...
                return (
                  <tr key={z.key} className="data-table-row border-b border-acb-100">
...
function OnOffCard({ records, loadLineupsForSeason, lineupsCache, loadingLineups }) {
  ...
  return (
    <div className={playerToolStyles.evidenceCard}>
      ...
      {hasAnyData && (
        <div className={playerToolStyles.tableFrame} tabIndex={0} aria-label="Tabla de impacto On/Off">
          <table className="data-table border-collapse">
            <thead>
              <tr className="bg-acb-100 text-xs font-semibold text-acb-700 uppercase border-b border-acb-300">
                <th className="data-table-sticky data-table-sticky-head data-col-season bg-acb-100 px-2 py-1.5 text-left whitespace-nowrap" rowSpan={2}>Temp.</th>
                <th className="data-table-sticky-after-season data-table-sticky-head data-col-team bg-acb-100 px-2 py-1.5 text-left whitespace-nowrap" rowSpan={2}>Equipo</th>
              ...
              <tr className="bg-acb-50 text-xs font-semibold text-acb-600 uppercase border-b border-acb-200">
            ...
            <tbody className="divide-y divide-acb-100">
              {rows.filter(r => r.found).map(r => {
                ...
                const drtgD = r.onDRtg != null && r.offDRtg != null ? r.onDRtg - r.offDRtg : null
                return (
                  <tr key={`${r.season}-${r.team}`} className="data-table-row border-b border-acb-100">
    ...
    <div className={playerToolStyles.evidenceCard}>
      <div className={playerToolStyles.evidenceHeader}>
        ...
        {clutchRows.length > 0 && (
          <button
            onClick={handleDownload}
            className={`${playerToolStyles.secondaryButton} ml-auto`}
            title="Descargar CSV"
          >
            <Download className="w-3.5 h-3.5" />
            CSV
            ...
            <thead>
              <tr className="bg-acb-100 border-b border-acb-300">
                {['Temporada','Equipo','PJ','Pts','Reb','Ast','Rob','Tap','Pér','T2%','3P%','TL%','eFG%','TS%','3PAr'].map(h => (
                  <th key={h} className={`px-3 py-2 text-xs font-semibold text-acb-700 uppercase tracking-wider whitespace-nowrap ${h === 'Temporada' || h === 'Equipo' ? 'text-left' : 'text-right'} ${h === 'Temporada' ? 'data-table-sticky data-table-sticky-head data-col-season bg-acb-100' : h === 'Equipo' ? 'data-table-sticky-after-season data-table-sticky-head data-col-team bg-acb-100' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-acb-100">
...
export default function PlayerProfile({ players, allPlayers = players, playerPhotos = {}, playerBio = {}, loadLineupsForSeason, lineupsCache, loadingLineups, loadClutchForSeason, clutchCache, loadingClutch }) {
  ...
  return (
    <PlayerToolShell
      ...
      {!selectedLicenseId && (
        <div className={playerToolStyles.emptyState}>
          Busca un jugador para abrir su trayectoria, su rol estadístico y el detalle de una temporada concreta.
        </div>
      )}
      ...
      {selectedLicenseId && allPlayerRecords.length > 0 && (
        <>
          ...
          <div className={playerToolStyles.profileActions}>
            ...
            {selectedSeason && selectedStage === 'regular' && (
              ...
              >
                ...
                <ArrowRight className="w-4 h-4 text-accent-500" aria-hidden="true" />
              </button>
            )}
          </div>
