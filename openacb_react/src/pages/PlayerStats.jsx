...
export default function PlayerStats({ players, playerBio = {} }) {
  ...
  return (
    <PlayerToolShell
      ...
      {/* Filters */}
      <div className={`${playerToolStyles.controlBand} ${playerToolStyles.statsControls}`}>
        <div className={playerToolStyles.controlRow}>
          ...
          {(viewMode === 'basic' || viewMode === 'advanced' || viewMode === 'misc') && (
            <div className="flex items-center gap-1.5">
              ...
              <div className="segmented-control" role="group" aria-label="Referencia del percentil">
                <button
                  onClick={() => updateQuery({ referencia: 'league' })}
                  aria-pressed={pctMode === 'league'}
                  className="segmented-option"
                >Liga</button>
                <button
                  onClick={() => updateQuery({ referencia: 'position' })}
                  aria-pressed={pctMode === 'position'}
                  className="segmented-option"
                >Posición</button>
              </div>
            </div>
          ...
          {filteredOutCount > 0 && (
            <button
              ...
              className={`${playerToolStyles.secondaryButton} ${showFilteredPlayers ? playerToolStyles.toggleActive : ''}`}
            >
      ...
      <div className={playerToolStyles.resultBar}>
        ...
        <button
          ...
          className={`${playerToolStyles.secondaryButton} disabled:cursor-not-allowed disabled:opacity-50`}
      ...
      {/* Table */}
      <section className={playerToolStyles.dataSection} aria-label="Resultados de jugadores">
        <div className={playerToolStyles.tableFrame} tabIndex={0} aria-label="Tabla de estadísticas de jugadores">
          <table className="data-table min-w-full">
            <thead>
              <tr className="bg-acb-100 border-b border-acb-300">
                <th className="data-table-head data-table-number data-table-sticky data-table-sticky-head data-col-rank bg-acb-100">
                  #
                </th>
                {displayColumns.map(col => (
                  <th
                    ...
                    className={`data-table-head
                      ...
                      ${col.key === 'playerFull' ? 'data-table-sticky-after-rank data-table-sticky-head data-col-player bg-acb-100' : col.key === 'team' ? 'data-col-team' : col.key === 'position' ? 'data-col-position' : col.key === 'games' ? 'data-col-games' : 'data-col-number'}
                      ${col.sortable ? 'cursor-pointer hover:bg-acb-200' : ''}`}
                  >
