    ...
    >
      ...
      <div className={styles.controlBand}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="segmented-control" role="group" aria-label="Métrica de rendimiento por cuarto">
            {[
      ...
      {isLoading && (
        <div className={styles.loadingState} role="status">Cargando datos por cuarto…</div>
      )}

      {/* quarter performance table */}
      {!isLoading && sortedTeams.length > 0 && (
        <section className={styles.dataSection} aria-labelledby="quarter-performance-title">
          <div className={styles.sectionHeader}>
            <div>
            <h2 id="quarter-performance-title">
              Rendimiento por cuarto
            </h2>
            <p>{viewMode === 'diff' ? 'Diferencial (pts/partido)' : viewMode === 'scored' ? 'Puntos anotados (media)' : 'Puntos recibidos (media)'} · Selecciona un equipo para abrir su desglose por tramo.</p>
            </div>
            <span className={styles.panelMeta}>{sortedTeams.length} equipos</span>
          </div>
          <div className={styles.tableFrame} tabIndex={0} aria-label="Rendimiento por cuarto de los equipos">
                  ...
                  return (
                    ...
                    >
                      <td className="data-table-cell data-table-identity data-table-sticky data-col-team">
                        <button
                          className={styles.tableTeamButton}
                          aria-expanded={isSelected}
                          aria-controls={isSelected ? 'quarter-team-detail' : undefined}
                          onClick={event => {
                            event.stopPropagation()
                            updateState({ teamId: isSelected ? null : teamIdByName.get(t.team) })
                          }}
                        >{t.team}</button>
                      </td>
                      {arr.map((v, qi) => {
      ...
      {/* team detail panel */}
      {teamDetail && (
        <section id="quarter-team-detail" className={styles.profileSection} aria-labelledby="quarter-detail-title">
          <div className={styles.profileSectionTitle}>
            <h2 id="quarter-detail-title">Detalle de {teamDetail.team}</h2>
            <p>{teamDetail.games} partidos analizados</p>
          </div>
          <div className={styles.metricGrid}>
          {/* quarter bar chart */}
          <div className={`${styles.panel} ${styles.detailPanel} p-5`}>
          ...
          <div className="space-y-6">
            {/* segment heatmap */}
            <div className={`${styles.panel} ${styles.detailPanel} p-4`}>
            ...
            {teamDetail.afterTimeout && (
              <div className={`${styles.panel} ${styles.detailPanel} p-4`}>
                ...
                <div className={styles.timeoutSummary}>
                  <div>
