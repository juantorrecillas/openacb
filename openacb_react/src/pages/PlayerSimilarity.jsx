  ...
  return (
    <PlayerToolShell
      ...
      {selectedLicenseId && availableSeasons.length > 0 && (
        <div className={`${playerToolStyles.profileActions} ${playerToolStyles.similarityActions}`}>
      ...
      {currentRecord && standoutCols.length > 0 && (
        <section className={playerToolStyles.similarityIdentity} aria-label="Rasgos del jugador seleccionado">
          <div className={playerToolStyles.similaritySubject}>
            <h2>{getPlayerDisplayName(currentRecord)}</h2>
            <p>{currentRecord.team} · {seasonLabel(currentRecord.season)} · {currentRecord.games} partidos</p>
            <p>Rasgos por encima de la media empleados para leer los resultados.</p>
          </div>
          <div className={playerToolStyles.similaritySignals}>
              {standoutCols.map(col => (
              <div key={col.key} className={playerToolStyles.signal}>
                <strong>{col.fmt(currentRecord[col.key])}</strong>
                <span>{col.label}</span>
              </div>
              ))}
          </div>
      ...
      {currentRecord && standoutCols.length > 0 && similarPlayers.length > 0 && (
        <section className={playerToolStyles.dataSection}>
          <div className={playerToolStyles.sectionHeader}>
            <div>
              <h2>20 perfiles más similares</h2>
              <p>La puntuación resume la cercanía del perfil completo; las columnas muestran los rasgos distintivos del jugador de referencia.</p>
            </div>
          </div>
          <div className={playerToolStyles.tableFrame} tabIndex={0} aria-label="Tabla de jugadores similares">
