...
scrape_season <- function(season_id,
                          data_dir = "./data/raw",
                          config_path = "./config/seasons.R") {
  # load local credentials relative to the config, regardless of working directory
  if (!nzchar(trimws(Sys.getenv("ACB_BEARER_TOKEN")))) {
    environ_path <- file.path(dirname(normalizePath(config_path, mustWork = TRUE)), "..", ".Renviron")
    if (file.exists(environ_path)) readRenviron(environ_path)
  }
  
  # Load configuration
  source(config_path)
  if (!nzchar(trimws(API_CONFIG$bearer_token))) {
    stop("Missing ACB_BEARER_TOKEN. Set it in the environment or openacb_api/.Renviron.", call. = FALSE)
  }
  season <- get_season_config(season_id)
